...
#include "caffe/layers/enh_softmax_loss_layer.hpp"
#include "caffe/util/math_functions.hpp"

namespace caffe {

template <typename Dtype>
void EnhSoftmaxWithLossLayer<Dtype>::LayerSetUp(
...
template <typename Dtype>
void EnhSoftmaxWithLossLayer<Dtype>::Reshape(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void EnhSoftmaxWithLossLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  caffe_copy(outer_num_ * inner_num_, label, label_fuben);  
  ...
  for (int i = 0; i < outer_num_/3*2; ++i) {   //batch  N 
    int mm_count = 0;
  ...
  for (int i = 0; i < outer_num_; ++i) {   
    ...
    if(batch_flag_data[i] == 1.0){
		for (int j = 0; j < inner_num_; j++) {
		  int label_value = label_fuben[i * inner_num_ + j];
		  ...
		  int idx = i * dim + label_value * inner_num_ + j;
		  if (weight_by_label_freqs_) {
...
template <typename Dtype>
void EnhSoftmaxWithLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    const Dtype* label_fuben   = label_fuben_.cpu_data();   
...
#ifdef CPU_ONLY
STUB_GPU(EnhSoftmaxWithLossLayer);
#endif

INSTANTIATE_CLASS(EnhSoftmaxWithLossLayer);
REGISTER_LAYER_CLASS(EnhSoftmaxWithLoss);
